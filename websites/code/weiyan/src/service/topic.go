...
func FindTopicPopular(tid string) (like int, hate int, err error) {
	condition := "tid=" + tid
	// 帖子信息
...
func IncrTopicView(tid string, uid int) (err error) {
	err = model.NewTopic().Where("tid="+tid).Increment("view", 1)
	if err != nil {
		logger.Errorln("更新帖子view数失败：", err)
	}
	return
}

func IncrTopicLike(tid string, uid int) (err error) {
	err = model.NewTopic().Where("tid="+tid).Increment("like", 1)
	if err != nil {
		logger.Errorln("更新帖子like数失败：", err)
	}
	return
}

func IncrTopicHate(tid string, uid int) (err error) {
	err = model.NewTopic().Where("tid="+tid).Increment("hate", 1)
	if err != nil {
		logger.Errorln("更新帖子hate数失败：", err)
	}
	return
}
